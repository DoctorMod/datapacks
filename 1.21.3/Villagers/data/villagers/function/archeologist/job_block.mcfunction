#Spawn Block Display
execute if entity @s[tag=!villager.firstrun] run setblock ~ ~ ~ loom{lock:{items:ender_dragon_spawn_egg}}
...
#Kill Block Display
execute at @s unless block ~1 ~-1 ~-1 loom run kill @s

#Unbind Block Display
execute at @e[type=villager,tag=villager.archeologist] as @s if score @s villager.id = @e[distance=..0.001,limit=1] villager.id at @s run data modify entity @s glow_color_override set value 0
